...
export type ICartMenuProps = {}
export const CartMenu: React.FC<ICartMenuProps> = () => {
  const router = useRouter();
  const dispatch = useDispatch();
  const cart = useSelector((state: any) => state.cart.cart);
  const isCartOpen = useSelector((state: any) => state.cart.isCartOpen);

  const totalPrice: number = cart.reduce((total: number, item: { count: number; attributes: { price: number } }) => {
